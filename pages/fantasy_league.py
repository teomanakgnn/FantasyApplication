        ...
        with tab2:
            if matchups:
                ...
                for match in matchups:
                    # GP Verilerini Çekelim (Yoksa 0 varsayalım)
                    gp_away = match['away_team']['stats'].get('GP', '0')
                    gp_home = match['home_team']['stats'].get('GP', '0')

                    with st.container(border=True):
                        # 3 Kolonlu Yapı: Deplasman - VS - Ev Sahibi
                        col1, col2, col3 = st.columns([1, 0.2, 1])
                        
                        # --- DEPLASMAN TAKIMI (SOL) ---
                        with col1:
